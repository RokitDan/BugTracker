        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    BTUser btUser = await _userManager.FindByEmailAsync(Input.Email);
                    var userId = await _userManager.GetUserIdAsync(btUser);
                    // Add new user to the designated project
                    await _projectService.AddUserToProjectAsync(btUser, Input.ProjectId);
